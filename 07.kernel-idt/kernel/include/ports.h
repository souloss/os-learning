...
/* PIC初始化序列 */
#define PIC_ICW1_ICW4 0x01      /* ICW1 (需要ICW4) */
#define PIC_ICW1_SINGLE 0x02    /* ICW1 (单PIC模式) */
#define PIC_ICW1_INTERVAL4 0x04 /* ICW1 (调用地址间隔4) */
#define PIC_ICW1_LEVEL 0x08     /* ICW1 (电平触发模式) */
#define PIC_ICW1_INIT 0x10      /* ICW1 (初始化) */
...
/* 端口I/O操作 */
// #define inb(port) ({ unsigned char _v; __asm__ __volatile__ ("inb %1, %0" : "=a" (_v) : "dN" ((port))); _v; })
// #define outb(port, val) __asm__ __volatile__("outb %0, %1" : : "a"((unsigned char)(val)), "dN"((port)))
// #define inw(port) ({ unsigned short _v; __asm__ __volatile__ ("inw %1, %0" : "=a" (_v) : "dN" ((port))); _v; })
// #define outw(port, val) __asm__ __volatile__("outw %0, %1" : : "a"((unsigned short)(val)), "dN"((port)))
// #define inl(port) ({ unsigned long _v; __asm__ __volatile__ ("inl %1, %0" : "=a" (_v) : "dN" ((port))); _v; })
// #define outl(port, val) __asm__ __volatile__("outl %0, %1" : : "a"((unsigned long)(val)), "dN"((port)))

// /* 等待I/O操作完成 */
#define io_wait() outb(IO_DELAY_PORT, 0)
